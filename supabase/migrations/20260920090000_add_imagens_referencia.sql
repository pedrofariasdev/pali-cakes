...
-- Coluna para guardar as fotos de referência/inspiração que o cliente
-- envia no checkout (caminhos dentro do bucket privado abaixo, não URLs
-- públicos — o admin gera um link temporário para as ver).
alter table public.encomendas
  add column if not exists imagens_referencia jsonb not null default '[]'::jsonb;

-- Bucket privado para as fotos de referência enviadas pelos clientes.
insert into storage.buckets (id, name, public)
values ('encomendas-referencias', 'encomendas-referencias', false)
on conflict (id) do nothing;

drop policy if exists "clientes enviam imagens de referencia" on storage.objects;
drop policy if exists "admin le imagens de referencia" on storage.objects;

-- Qualquer visitante (mesmo sem sessão) pode enviar fotos ao finalizar
-- uma encomenda — é exactamente o mesmo cliente anónimo que já cria a
-- encomenda através da função criar_encomenda.
create policy "clientes enviam imagens de referencia"
on storage.objects
for insert
to anon, authenticated
with check (
  bucket_id = 'encomendas-referencias'
);

-- Só o admin pode listar/ver as fotos (via URL assinado, o bucket não é público).
create policy "admin le imagens de referencia"
on storage.objects
for select
to authenticated
using (
